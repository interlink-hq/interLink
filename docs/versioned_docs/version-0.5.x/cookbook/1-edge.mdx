...
# Test logging functionality (requires CSR approval)
kubectl logs test-tunnel
```

:::warning CSR Manual Approval Required

⚠️ **IMPORTANT**: If `kubectl logs` fails, you need to manually approve Certificate Signing Requests (CSRs). Without CSR approval, **log access will not work**.
...
3. **Verify logs are now accessible**:
   ```bash
   kubectl logs test-tunnel
   ```
