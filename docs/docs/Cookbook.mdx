...
<Tabs groupId="scenarios">
  ...
  <TabItem value="tunnel" label="Tunneled">
    ...
    - Start the tunnel 
      ...
      ## If you want to remove the secure false, you should enable HostKey check (more advanced manual setup)
    
      $HOME/.interlink/bin/ssh-tunnel -secure false -addr $CLUSTER_PUBLIC_IP:$SSH_TUNNEL_NODE_PORT -keyfile $PRIV_KEY_FILE -user interlink -rport 3000 -lsock plugin.sock  &> $HOME/.interlink/logs/ssh-tunnel.log &
