...
<Tabs groupId="scenarios">
  <TabItem value="edge" label="Edge node">
    ...
    For a case where the `my-node` will contact an edge service that will be listening con `PUBLIC_IP` and `API_PORT` authenticating requests from an OIDC provider `https://my_oidc_idp.com` the configuration file should be edited like the following, as a good enough starting point:
...
<Tabs groupId="plugins">
  <TabItem value="docker" label="Docker" default>
    ...
    - Create a configuration file: 
      ...
      ## Multi user host
      SidecarURL: "unix:///home/myusername/plugin.sock"
      InterlinkPort: "0"
      SidecarPort: "0"
  ...
  <TabItem value="slurm" label="SLURM">
    ...
    - Start the plugins passing the configuration that you have just created:
      ...
      ```bash
      export SLURMCONFIGPATH=./interlink/manifests/plugin-config.yaml
      $HOME/.interlink/bin/plugin &> $HOME/.interlink/logs/plugin.log &
      echo $! > $HOME/.interlink/plugin.pid     
      ```
...
```
curl -v --unix-socket ${HOME}/.interlink/interlink.sock  http:/pinginterlink
```
...
<Tabs groupId="scenarios">
  <TabItem value="edge" label="Edge node">
    ...
    ```bash
     helm upgrade --install --create-namespace -n interlink my-node oci://ghcr.io/intertwin-eu/interlink-helm-chart/interlink --values ./interlink/manifests/values.yaml 
    ```
