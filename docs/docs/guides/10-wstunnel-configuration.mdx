...
Network:
  ...
  IngressTLS: true                                     # Optional: expose tunnel ingress with TLS
  IngressClusterIssuer: "lets-issuer"                 # Optional: cert-manager ClusterIssuer
...
| `IngressTLS` | `bool` | No | `false` | Adds TLS to generated ingresses and makes the default client command use `wss://...:443` |
| `IngressClusterIssuer` | `string` | No | `lets-issuer` when TLS is enabled | cert-manager ClusterIssuer used by generated TLS ingresses |
...
```
{resource-name}-{wstunnel-namespace}.{WildcardDNS}
```

Examples:
- Pod `web-server` in namespace `default` → `web-server-default-default-wstunnel.tunnel.example.com`
- Pod `api-gateway` in namespace `apps` → `api-gateway-apps-apps-wstunnel.tunnel.example.com`
...
./wstunnel client --http-upgrade-path-prefix a1b2c3d4e5f6 -R tcp://[::]:8080:localhost:8080 -R tcp://[::]:9090:localhost:9090 ws://my-pod-default-default-wstunnel.tunnel.example.com:80
```
...
2. **Connect**: Establishes the websocket tunnel with:
   ...
   - `ws://ENDPOINT:80`: Websocket endpoint using generated hostname, or `wss://ENDPOINT:443` when `IngressTLS` is enabled
...
./wstunnel client --http-upgrade-path-prefix randompassword \
  ...
  ws://my-pod-default-default-wstunnel.tunnel.example.com:80
```
...
3. **DNS resolution issues**
   ```bash
   # Test DNS resolution
   nslookup my-pod-default-default-wstunnel.tunnel.example.com
...
curl -i -N -H "Connection: Upgrade" -H "Upgrade: websocket" \
  ...
  http://my-pod-default-default-wstunnel.tunnel.example.com/your-random-password
