...
Tracing:
  Enabled: false # Export API server traces
  Detailed: false # Add opt-in high-cardinality diagnostic attributes
...
| Field      | Type | Default | Description |
| ---------- | ---- | ------- | ----------- |
| `Enabled`  | bool | `false` | Export OpenTelemetry traces from the interLink API server |
| `Detailed` | bool | `false` | Add container/image names, Kubernetes metadata keys, referenced ConfigMap/Secret names, cache decisions, client details, and response-mode diagnostics |

`Tracing.Detailed` intentionally does not export HTTP bodies, Secret contents,
ConfigMap contents, environment variable values, container commands, or
arguments. Because detailed attributes can still have high cardinality and may
contain workload identifiers, enable this mode temporarily or only where the
telemetry backend has appropriate access controls.
...
| `ENABLE_TRACING`      | `Tracing.Enabled`  | Enable or disable API server tracing (`true`, `false`, `1`, or `0`)          |
| `ENABLE_DETAILED_TRACING` | `Tracing.Detailed` | Override detailed diagnostic attributes (`true`, `false`, `1`, or `0`)   |
...
The API server accepts and propagates W3C `traceparent` headers. Handler spans
are server spans, and calls to the configured plugin or job-script builder are
recorded as child client spans. Standard HTTP attributes such as
`http.request.method`, `http.route`, `http.response.status_code`, request/response
body sizes, network endpoint information, and `error.type` are included alongside
the legacy `exit.code` field.
