...
- **Core Configuration**: Basic interLink and sidecar connection settings
...
SidecarURL: "http://localhost"     # URL of the sidecar plugin
SidecarPort: "4000"                # Port of the sidecar plugin
...
| `SidecarURL` | string | `"http://localhost"` | Base URL of the sidecar plugin |
| `SidecarPort` | string | `"4000"` | Port of the sidecar plugin |
...
The `JobScriptBuildConfig` section configures container runtime options:

#### Singularity Hub Configuration

| Field | Type | Description |
|-------|------|-------------|
| `server` | string | Singularity Hub server URL |
| `master_token` | string | Authentication token for private repositories |
| `cache_validity_seconds` | int | Cache validity duration in seconds |
...
| `SIDECARURL` | `SidecarURL` | Override sidecar URL |
| `SIDECARPORT` | `SidecarPort` | Override sidecar port |
