...
The Virtual Kubelet (VK) and interLink API server are instrumented with
...
The following table lists the main spans generated by the VK and API server:

| Span Name      | Description                                                                                        | Attributes                                                                    |
| -------------- | -------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| CreateHttpCall | Span that represents the HTTP call made by the VK to the Interlink API to create a pod.            | pod.name, pod.namespace, start.timestamp, end.timestamp, duration, exit.code |
| DeleteHttpCall | Span that represents the HTTP call made by the VK to the Interlink API to delete a pod.            | pod.name pod.namespace, start.timestamp, end.timestamp, duration, exit.code  |
| StatusHttpCall | Span that represents the HTTP call made by the VK to the Interlink API to get the status of a pod. | pod.name pod.namespace, start.timestamp, end.timestamp, duration, exit.code  |
| LogHttpCall    | Span that represents the HTTP call made by the VK to the Interlink API to get the logs of a pod.   | pod.name pod.namespace, start.timestamp, end.timestamp, duration, exit.code  |
| PingHttpCall   | Span that represents the HTTP call made by the VK to the Interlink API check if the API is alive.  | start.timestamp, end.timestamp, duration, exit.code                          |
...
| CreateAPI      | API server processing of a pod creation request.                                                    | HTTP metadata, pod identity, workload/object counts, retrieval timings         |
| DeleteAPI      | API server processing of a pod deletion request.                                                    | HTTP metadata, pod identity                                                     |
| StatusAPI      | API server status lookup and cache processing.                                                      | HTTP metadata, pod counts, cache hit/miss and plugin-query counts               |
| GetLogsAPI     | API server log request validation and streaming.                                                    | HTTP metadata, pod/container identity, log options                              |
| PingAPI        | API server health check against the plugin.                                                         | HTTP metadata                                                                   |
| UpdateCacheAPI | API server cache invalidation.                                                                       | HTTP metadata, pod UID                                                          |
| HTTP GET/POST  | Child client span for a plugin or job-script-builder request.                                       | destination, request/response sizes, upstream status                            |

API server tracing can be configured in `InterLinkConfig.yaml`:
...
Set `Detailed: true` to add high-cardinality diagnostics such as container and
image names, metadata keys, referenced object names, cache decisions, and
