...
InterLink API and the plugin can be arranged in two different ways across the
local cluster and the remote part:

- both deployed remote (**[Edge-node](#edge-node)**)
- API deployed in the cluster (**[In-cluster](#in-cluster)**), with the plugin
  either beside it or on the remote system, reached over an SSH tunnel
...
The first two rows are both *in-cluster* deployments — the API server sits in the
cluster either way, and only the plugin moves. The third is the *edge-node* one.
...
server inside the Kubernetes cluster, so nothing has to be installed on an edge
node and no interLink component is exposed to the internet.

The plugin either runs beside them in the cluster, or on the remote system — on a
login node, for instance — with the API server reaching it through an SSH tunnel.
That second variant suits multi-user hosts, and sites that allow outbound SSH but
will not let you expose a port.
