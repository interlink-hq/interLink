...
HOST_DNS_SERVERS=$(awk '/^nameserver/ {print $2}' /etc/resolv.conf)

{
  ...
  for dns in $HOST_DNS_SERVERS; do
    case "$dns" in
      "{{.DNSServiceIP}}"|"1.1.1.1"|"8.8.8.8") continue ;;
    esac
    echo "nameserver $dns" >> /tmp/etc-override/resolv.conf
  done
...
export LOCALDOMAIN=/tmp/etc-override/resolv.conf
