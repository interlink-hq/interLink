...
func (r *FakeImageService) ListImages(_ context.Context, filter *runtimeapi.ListImagesRequest) (*runtimeapi.ListImagesResponse, error) {
	r.Lock()
	defer r.Unlock()
	...
	for _, img := range r.Images {
		if filter != nil && filter.Filter.Image != nil {
			if !stringInSlice(filter.Filter.Image.Image, img.RepoTags) {
				continue
			}
		}
...
func (r *FakeImageService) ImageStatus(_ context.Context, image *runtimeapi.ImageStatusRequest) (*runtimeapi.ImageStatusResponse, error) {
	r.Lock()
	defer r.Unlock()
	...
	return &runtimeapi.ImageStatusResponse{Image: r.Images[image.Image.Image]}, nil
}

// PullImage emulate pulling the image from the FakeImageService.
func (r *FakeImageService) PullImage(_ context.Context, image *runtimeapi.PullImageRequest) (*runtimeapi.PullImageResponse, error) {
	r.Lock()
	defer r.Unlock()
	...
	r.pulledImages = append(r.pulledImages, &pulledImage{imageSpec: image.Image, authConfig: image.Auth})
...
func (r *FakeImageService) RemoveImage(_ context.Context, image *runtimeapi.RemoveImageRequest) (*runtimeapi.RemoveImageResponse, error) {
	r.Lock()
	defer r.Unlock()

	r.Called = append(r.Called, "RemoveImage")
	if err := r.popError("RemoveImage"); err != nil {
		return &runtimeapi.RemoveImageResponse{}, err
	}

	// Remove the image
	delete(r.Images, image.Image.Image)

	return &runtimeapi.RemoveImageResponse{}, nil
}

// ImageFsInfo
func (r *FakeImageService) ImageFsInfo(context.Context, *runtimeapi.ImageFsInfoRequest) (*runtimeapi.ImageFsInfoResponse, error) {
